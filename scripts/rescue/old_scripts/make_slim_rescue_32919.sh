...
# USAGE: ./make_slim_rescue_32919.sh [Ns] [Ts] [Nb] [nM] [Tr] [h]


# Set Na, the source population size
Ns=${1}
...
# Make script
cat > slim_rescue_${Ns}Ns_${Ts}Ts_${Nb}Nb_${nM}nM_${Tr}Tr_h${h}_32919.slim << EOM
...
100001:$((100000+${Ts})) late() {
	if (sim.generation % 50 == 0) {
		i = sample(p2.individuals, sampleSize, F);
		...
		cat(sim.generation + "," + p2.individuals.size() + "," + mean(fitness_population) + "," + mean(ind_heteroz) + "," + mean(ROH_length_sumPerInd)/seqLength + "," + sum(Num_VstrDel_muts)/i.size() + "," + sum(Num_strDel_muts)/i.size() + "," + sum(Num_modDel_muts)/i.size() + "," + sum(Num_wkDel_muts)/i.size() + "," + "\n");
...
// track statistics for p2 and p3 every generation
$((100001+${Ts})):$((100001+${Ts}+5000)) late() {
	...
	for (pop in pops){
		...
		if (pop.individuals.size() > 1){
			if(pop.individuals.size() > sampleSize){
			...
			cat(pop.individuals.size() + "," + mean(fitness_population) + "," + mean(ind_heteroz) + "," + mean(ROH_length_sumPerInd)/seqLength + "," + sum(Num_VstrDel_muts)/i.size() + "," + sum(Num_strDel_muts)/i.size() + "," + sum(Num_modDel_muts)/i.size() + "," + sum(Num_wkDel_muts)/i.size() + ",");
	
		}
	
	}
	if(pop.individuals.size() < 2){
