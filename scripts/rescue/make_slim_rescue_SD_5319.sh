...
# Make script
cat > slim_rescue_${Ns}Ns_${Ts}Ts_${Nb}Nb_${nM}nM_${nR}nR_h${h}_SD_5319.slim << EOM
...
$((100001+${Ts})):$((100001+${Ts}+5000)) late() {      
        if(p3.individualCount <=5 & sim.getValue("rescueCount") < numRescue){
                ...
                for (individual in i) {
                        ...
                        individual.tag = sum(s<=-0.01); //use the tag property of each individual to store their number of deleterious variants
                }
