...
# USAGE: ./make_slim_rescue_5319.sh [Ns] [Ts] [Nb] [nM] [nR] [h]
...
# Make script
cat > slim_rescue_${Ns}Ns_${Ts}Ts_${Nb}Nb_${nM}nM_${nR}nR_h${h}_NS_5319.slim << EOM
...
$((100001+${Ts})):$((100001+${Ts}+5000)) late() {      
        if(p3.individualCount <=5 & sim.getValue("rescueCount") < numRescue){
                ...
                // here I am looping over all individuals in the source population, identifying those with the lowest number of NS variants,and selecting those for rescue

		//i = p2.individuals;
                i = sample(p2.individuals, 100, F); // subsampling important for het/NS which is VERY slow but not for deleterious variants
 		...
 		m_uniq = unique(m); // get rid of redundant muts
			...
			individual.tag = sum(indm_uniq.mutationType == m1); // set individual.tag as number of NS muts

                }
                
                //sort the individuals by their number of deleterious variants and then take those with the fewest as migrants
                inds_NS = sortBy(i, "tag", ascending = T); // be careful of order of sort - note that default is ascending
                migrants = inds_NS[0:(numMigrants-1)];
