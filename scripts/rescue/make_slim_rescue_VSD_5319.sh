...
# Make script
cat > slim_rescue_${Ns}Ns_${Ts}Ts_${Nb}Nb_${nM}nM_${nR}nR_h${h}_VSD_5319.slim << EOM
...
$((100001+${Ts})):$((100001+${Ts}+5000)) late() {      
        if(p3.individualCount <=5 & sim.getValue("rescueCount") < numRescue){
                ...
                for (individual in i) {
                        ...
                        Num_VstrDel_muts = c(Num_VstrDel_muts, sum(s<=-0.05));
                        //Num_strDel_muts = c(Num_strDel_muts, sum(s<=-0.01));
                        individual.tag = sum(s<=-0.05); //use the tag property of each individual to store their number of deleterious variants
                }
                
                //sort the individuals by their number of deleterious variants and then take those with the fewest as migrants
                inds_delMuts = sortBy(i, "tag");
                migrants = inds_delMuts[0:(numMigrants-1)];
