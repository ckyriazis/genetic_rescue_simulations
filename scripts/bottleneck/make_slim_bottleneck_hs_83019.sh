...
# USAGE: ./make_slim_bott_4319.sh [Na] [Nb] [nF]
...
# Make script
cat > slim_bottleneck_${Na}Na_${Nb}Nb_${nF}nF_hs_83019.slim << EOM
...
function (s) getStats(o pop, i sampSize)
{
	...
	return(pop.individuals.size() + "," + mean(ind_het) + "," + mean(ROH_length_sumPerInd)/seqLength + "," + mean(Num_VstrDel_muts) + "," + mean(Num_strDel_muts)+ "," + mean(Num_modDel_muts) + "," + mean(Num_wkDel_muts));
}
...
$((${Na}*10+1)) early(){
	...
	cat("gen,K3,p_death,popSizeP3,meanHet,FROH,avgVStrDel,avgStrDel,avgModDel,avgWkDel" + "\n");
