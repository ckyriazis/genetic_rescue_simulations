...
# USAGE: ./make_slim_bott_4319.sh [Na] [Nb] [nF] [h] [g]
...
# Set g, the number of genes
g=${5}

# Make script
cat > slim_bottleneck_${Na}Na_${Nb}Nb_${nF}nF_h${h}_g${g}_72219.slim << EOM
...
initialize() {
	...
	defineConstant("g",${g}); //number of genes
		...
		if (size(uniquemuts) > 0){
			for (u in uniquemuts){
				...
				if ((m1.dominanceCoeff == 0.0) & (size(uu) == 2)) {
					...
					fitness =  product(1 + uu.selectionCoeff * m1.dominanceCoeff);
				}
...
// print stats only for first generation after bottleneck
$((${Na}*10+1)) late() {
...
$((${Na}*10+2)):$((${Na}*10+20000)) late() {

	// Commenting all this out due to problems with the getStats function with a small number of genes
	// the number of unique mutations goes to 0 which breaks the function
	...
	stats_P3 = c("NA,NA,NA,NA,NA,NA,NA,NA");
